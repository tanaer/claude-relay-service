        ...
        <div class="relative">
          <div class="mb-2 flex items-center justify-between">
            ...
            <span class="text-sm font-medium text-gray-900">
              ...
              {{ formatTokenDisplay(getDailyTokenLimit()) }}
            </span>
          </div>
          ...
          <div class="mt-2 flex items-center justify-between text-xs text-gray-500">
            ...
            <span v-if="statsData?.usage?.daily?.tokens">
              今日已用: {{ formatTokenDisplay(statsData.usage.daily.tokens) }}
            </span>
          </div>
        </div>
      </div>
...
const getCurrentDayTokens = () => {
  // 优先使用statsData中的当日token使用量，如果没有则使用currentPeriodData中的tokens
  return parseInt(statsData.value?.usage?.daily?.tokens || currentPeriodData.value?.tokens || 0)
}

// 获取每日Token限制
const getDailyTokenLimit = () => {
  // 从API Key配置中获取token限制，如果没有配置则返回一个默认的大值
  return parseInt(
    statsData.value?.limits?.tokenLimit || statsData.value?.limits?.dailyTokenLimit || 70000000
  )
}
...
const tokenProgress = computed(() => {
  ...
  const tokenLimit = getDailyTokenLimit()

  if (tokenLimit === 0) return 0

  const progress = (todayTokens / tokenLimit) * 100
